...
const STATUS_STYLES = {
  ...
  stage1_pending_user_approval: { bg: '#fef3c7', color: '#b45309', label: 'Pending User Approval' },
  stage1_user_rejected: { bg: '#fde8e8', color: '#b91c1c', label: 'User Rejected' },
  stage1_approved: { bg: '#e3f6e8', color: '#1f7a3f', label: 'Stage 1 - Approved' },
  stage1_rejected: { bg: '#fde8e8', color: '#b91c1c', label: 'Stage 1 - Rejected' },
...
const STATUS_OPTIONS = [
  ...
  { value: 'stage1_pending_user_approval', label: 'Pending User Approval' },
  { value: 'stage1_user_rejected', label: 'User Rejected' },
  { value: 'stage1_approved', label: 'Stage 1 - Approved' },
  { value: 'stage1_rejected', label: 'Stage 1 - Rejected' },
  { value: 'stage1_completed', label: 'Stage 1 - Completed' },
]
...
const OfficerDashboard = () => {
  const { user, officerInvoices, officerInvoicesPagination, refreshOfficerInvoices } = useApp()
  ...
  }, [user?.id, page, limit, status, search, refreshOfficerInvoices])
  ...
  const totalPages = officerInvoicesPagination?.totalPages || 1
  const currentPage = officerInvoicesPagination?.currentPage || page
  const totalRecords = officerInvoicesPagination?.totalRecords || 0
  ...
  return (
    <>
      <style>{`
        ...
        /* Mobile card list — hidden on wider screens */
          ...
          <>
            ...
            <div className="od-table-wrap">
              <table className="od-table">
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.id} onClick={() => navigate(`/officer/invoices/${row.id}`)}>
            ...
            <div className="od-pagination">
              ...
              <div className="od-pagination-btns">
                <button
                  ...
                  disabled={!officerInvoicesPagination?.hasPreviousPage}
                >
                  Previous
                </button>
                <button
                  ...
                  disabled={!officerInvoicesPagination?.hasNextPage}
                >
                  Next
                </button>
              </div>
