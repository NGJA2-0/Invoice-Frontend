...
const Stage2Dashboard = () => {
  const { user, stage2OfficerInvoices, refreshStage2OfficerInvoices } = useApp()
  ...
  useEffect(() => {
    if (!user?.id) return
    if (stage2OfficerInvoices.length > 0) {
      setLoading(false)
      return
    }
    setLoading(true)
  ...
  }, [user?.id, stage2OfficerInvoices.length, refreshStage2OfficerInvoices])
