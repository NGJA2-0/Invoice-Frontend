...
const Stage3Dashboard = () => {
  const { user, stage3OfficerInvoices, refreshStage3OfficerInvoices } = useApp()
  ...
  useEffect(() => {
    if (!user?.id) return
    if (stage3OfficerInvoices.length > 0) {
      setLoading(false)
      return
    }
    setLoading(true)
  ...
  }, [user?.id, stage3OfficerInvoices.length, refreshStage3OfficerInvoices])
