...
const STATUS_STYLES = {
  stage3_in_progress: { bg: '#f3e8ff', color: '#7e22ce', label: 'Stage 3 - In Progress' },
  stage3_pending_user_approval: { bg: '#fef3c7', color: '#b45309', label: 'Pending User Approval' },
  stage3_user_rejected: { bg: '#fde8e8', color: '#b91c1c', label: 'User Rejected' },
  stage3_rejected: { bg: '#fde8e8', color: '#b91c1c', label: 'Stage 3 - Rejected' },
  stage3_completed: { bg: '#e3f6e8', color: '#1f7a3f', label: 'Stage 3 - Completed' },
...
const STATUS_OPTIONS = [
  ...
  { value: 'stage3_pending_user_approval', label: 'Pending User Approval' },
  { value: 'stage3_user_rejected', label: 'User Rejected' },
  { value: 'stage3_rejected', label: 'Stage 3 - Rejected' },
  { value: 'stage3_completed', label: 'Stage 3 - Completed' },
]
...
const Stage3Dashboard = () => {
  const { user, stage3OfficerInvoices, stage3OfficerInvoicesPagination, refreshStage3OfficerInvoices } = useApp()
  ...
  }, [user?.id, page, limit, status, search, refreshStage3OfficerInvoices])

  const rows = useMemo(
    () =>
      (stage3OfficerInvoices || []).map((inv) => ({
  ...
  const totalPages = stage3OfficerInvoicesPagination?.totalPages || 1
  const currentPage = stage3OfficerInvoicesPagination?.currentPage || page
  const totalRecords = stage3OfficerInvoicesPagination?.totalRecords || 0
          ...
          <>
            ...
            <div className="od-table-wrap">
              <table className="od-table">
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.id} onClick={() => navigate(`/officer3/invoices/${row.id}`)}>
            ...
            <div className="od-cards">
              {rows.map((row) => (
                <div
                  key={row.id}
                  className="od-card"
                  onClick={() => navigate(`/officer3/invoices/${row.id}`)}
                >
            ...
            <div className="od-pagination">
              ...
              <div className="od-pagination-btns">
                <button
                  ...
                  disabled={!stage3OfficerInvoicesPagination?.hasPreviousPage}
                >
                  Previous
                </button>
                <button
                  ...
                  disabled={!stage3OfficerInvoicesPagination?.hasNextPage}
                >
                  Next
                </button>
              </div>
